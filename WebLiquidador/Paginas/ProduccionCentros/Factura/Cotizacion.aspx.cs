    ...
    public partial class Cotizacion : System.Web.UI.Page
    {
        ...
        protected void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection con = new SqlConnection(CadenaConexion))
            {
                try
                {
                    var Textocmd = "[CentroFormacion].[Usp_CotizacionInsert]";
                    SqlCommand cmd = new SqlCommand(Textocmd, con);
                    cmd.CommandType = System.Data.CommandType.StoredProcedure;

                    cmd.Parameters.Add("@NumeroCotizacion", SqlDbType.BigInt).Value = Convert.ToInt64(Session["_NumeroCotizacion"].ToString());
                    ...
                    if (txtContacto.Text == "")
                    {
                        cmd.Parameters.Add("@Contacto", SqlDbType.Text).Value = System.DBNull.Value;
                    }
                    else
                    {
                    ...
                    SqlParameter outputIdParam = new SqlParameter("@IdCotizacion", SqlDbType.Int) { Direction = ParameterDirection.Output };
                    cmd.Parameters.Add(outputIdParam);

                    int vResultado;
                    con.Open();
                    cmd.ExecuteNonQuery();
                    vResultado = Convert.ToInt32(cmd.Parameters["@IdCotizacion"].Value);
                    con.Close();
                    Session["_IdCotizacion"] = vResultado.ToString();

                    Grabar_CotizacionDetalle();

                    Alerta("Cotización Guardada Correctamente");
        ...
        protected void Grabar_CotizacionDetalle()
        {
            var CadenaConexion = ConfigurationManager.ConnectionStrings["CentroFormacion"].ToString();
            using (SqlConnection con = new SqlConnection(CadenaConexion))
            {
                try
                {
                    var Textocmd = "[CentroFormacion].[Usp_CotizacionDetalleInsert]";
                    SqlCommand cmd = new SqlCommand(Textocmd, con);
                    cmd.CommandType = System.Data.CommandType.StoredProcedure;
                    ...
                    foreach (GridViewRow row in grvProductosCentro.Rows)
                    {
                        ...
                        cmd.Parameters.Add("@IdCotizacion", SqlDbType.Int).Value = Convert.ToInt32(Session["_IdCotizacion"].ToString());
        ...
        protected void GrabarNumeroCotizacion()
        {
            var CadenaConexion = ConfigurationManager.ConnectionStrings["CentroFormacion"].ToString();
            using (SqlConnection con = new SqlConnection(CadenaConexion))
            {
                ...
                {
                    long vResultado;
                    var Textocmd = "[CentroFormacion].[Usp_NumeroCotizacionUpdate]";
                    SqlCommand cmd = new SqlCommand(Textocmd, con);
                    SqlParameter outputIdParam = new SqlParameter("@NumeroCotizacion", SqlDbType.BigInt) { Direction = ParameterDirection.Output };
                    cmd.Parameters.Add("@IdCentroFormacion", SqlDbType.Int).Value = Convert.ToInt32(Session["_IdCentroFormacion"].ToString());
                    ...
                    con.Open();
                    cmd.ExecuteNonQuery();
                    vResultado = Convert.ToInt64(cmd.Parameters["@NumeroCotizacion"].Value);
                    con.Close();
                    Session["_NumeroCotizacion"] = vResultado.ToString();
                }
                catch (SqlException ex)
                {
                    Alerta("Error: " + ex.Message);
        ...
        protected void btnAgregar_Click(object sender, EventArgs e)
        {
            ...
            if (ddlTipoCliente.SelectedValue == "3" && txtIdentificacion.Text == "899999034")
            {
                Alerta("Atención: El Número de Identificación No Es Permitido para este Tipo de Cliente");
                ddlTipoCliente.Focus();
                return;
            ...
            {
                Alerta("Atención: El precio de Cotizacion no puede ser menor o igual a Cero");
                txtPrecio.Focus();
                return;
            }
            ...
            {
                Alerta("Atención: El Precio de Cotización no puede ser Menor a: " + txtPrecioMinimo.Text);
                txtPrecio.Focus();
                return;
            }
            ...
            {
                Alerta("Atención: El Precio de Cotización no puede ser Mayor a: " + txtPrecioMaximo.Text);
                txtPrecio.Focus();
                return;
            }
        ...
        protected void btnActualizaProducto_Click(object sender, EventArgs e)
        {
            ...
            {
                Alerta("Error, el Precio de Cotización debe ser Igual o Mayor al Precio Mínimo");
                txtPrecio.Focus();
                return;
            }
        ...
        {
            Response.Redirect("~/Paginas/ProduccionCentros/Factura/ImprimirCotizacion.aspx", false);
        }
        ...
        protected void ddlTipoCliente_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {//Sena Proveedor Sena
                ...
                txtNombre.Attributes.Add("placeholder", "Centro al qué le Cotiza");
