    ...
    public partial class CargarArchivo : System.Web.UI.Page
    {
        ...
        public void ImportDataFromExcel(string excelFilePath)
        {
            try
            {
                ...
                string ssqltable = "[Siif].[DeContado" + Session["_CodigoCentroFormacion"].ToString() + "]";
                ...
                if (IsPostBack && FuCargarArchivo.HasFile)
                {
                    if (Path.GetExtension(FuCargarArchivo.FileName).Equals(".xlsx"))
                    {
                        ...
                        if (excel.Workbook.Worksheets[1].Name.ToString() != "DeContado")
                        {
                            lblNombreArchivo.Text = excel.Workbook.Worksheets[1].Name.ToString();
                            Alerta("Error: El Archivo No es una archivo de Facturas Electrónica de Contado");
                            return;
                        }
                        var table = ssqltable;
        ...
        private void BuscarControlCargue()
        {
            ...
            using (SqlConnection con = new SqlConnection(CadenaConexion))
            {
                var Textocmd = "[CentroFormacion].[Usp_BuscarCargueFacturaElectronicaSiif]";
                SqlCommand cmd = new SqlCommand(Textocmd, con);
                cmd.CommandType = System.Data.CommandType.StoredProcedure;
            ...
            {
                Alerta("Atención El Archivo de Factura Electrónica de Contado no es de su Centro Formación");
        ...
        protected void btnEsCorrecto_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection con = new SqlConnection(CadenaConexion))
            {
                try
                {
                    ...
                    cmd.Parameters.Add("@IdTipoFacturaElectrónicaSiif", SqlDbType.Int).Value = 1;
        ...
        protected void ProcesarRegistros()
        {
            ...
            using (SqlConnection con = new SqlConnection(CadenaConexion))
            {
                ...
                {
                    int vResultado;
                    var Textocmd = "[CentroFormacion].[Usp_ProcesarFacturaElectronicaSiifXTipoFactura]";
                    SqlCommand cmd = new SqlCommand(Textocmd, con);
                    cmd.CommandType = System.Data.CommandType.StoredProcedure;
        ...
        {
            Response.Redirect("~/Paginas/ProduccionCentros/Siif/Facturas/DeContado/CargarArchivo.aspx", false);
        }
