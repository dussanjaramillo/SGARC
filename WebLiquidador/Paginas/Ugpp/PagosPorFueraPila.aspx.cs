...
{
    public partial class PagosPorFueraPila : System.Web.UI.Page
    {
        ...
        protected void btnExportar_Click(object sender, EventArgs e)
        {
            try
            {
                string PathArchivo = "SENA_PASENA_1003_" + txtFechaInicial.Text.ToString()+"_"+txtFechaFinal.Text.ToString() + "_PagosPorFueradePila.txt";
                //string PathArchivo = "4000_899999034_" + txtFechaInicial.Text.ToString()+".str";
                var CadenaConexion = ConfigurationManager.ConnectionStrings["CentroFormacion"].ToString();
                using (SqlConnection con = new SqlConnection(CadenaConexion))
                {
                    var Textocmd = "[Ugpp].[Usp_BuscarPagosPorFueradePila]";
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        if (ddlEncabezado.SelectedValue == "Si")
                        {
        ...
        {
            Response.Redirect("~/Paginas/Ugpp/PagosPorFueraPila.aspx", false);
        }
    }
}